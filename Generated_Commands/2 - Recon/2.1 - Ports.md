...
```bash
docker run rustscan/rustscan -a ${box_ip} | tee ${loc}/1-recon/rustscan_init.md
```
...
```bash
docker run rustscan/rustscan -a ${box_host} | tee /opt/THM/Whiterose/1-recon/rustscan_init_host.md
```
