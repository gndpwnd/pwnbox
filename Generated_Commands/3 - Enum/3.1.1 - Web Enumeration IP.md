...
```bash
docker run -it --rm wpscanteam/wpscan ${WPAPITOKEN} --url ${ipurl}:${web1} -f cli-no-color -o ${loc}/2-enum/web/wpscan_ip.md  --enumerate u,m,ap,at,tt,cb,dbe --plugins-detection mixed 
```

```bash
docker run -it --rm wpscanteam/wpscan ${WPAPITOKEN}--url ${ipurl}:${web1} -U ${folder_names[1]}/wp_users.md -P /usr/share/wordlists/rockyou.txt -o ${folder_names[1]}/web/wpscan_ip_brute.md
